...
class Sock{
    ...
    */
    ...
    //用户加入或client发送信息
    //其实是预处理，接收client发送的信息并根据其内容对将要发送的信息加入数组
    function proSend($k,$msg){
        ...
        if($g['type']=='add'){
            //第一次进入,添加名字，把区号保存在相应的users里面
            $this->users[$k]['area']=$g['area'];
            // $ar['type']='add';
            // $ar['name']=$g['area'];
            // $key='all';
            $this->e("add:".$g['area']);
            return;
        }else if($g['type']=='update'){
            //发送信息行为，1区发给2区
            $ar['type']="update";
            $target=explode(",",$g['target']);//目标area
            $ar['msg']=$g['msg'];
            //推送信息
            foreach ($target as $t) {
                $this->send($k,$ar,$t);
    ...
    function getusers($key='all'){
        $ar=array();
        if($key=='all'){
            foreach($this->users as $k=>$v){
                $ar[]=array('to'=>$k,'area'=>$v['area']);
            }
        }else{
            foreach($this->users as $k=>$v){
                //取出第$key区的socket
    ...
    //$k 发信息人的socketID $key接受人的 area名 ，根据这个socketID可以查找相应的client进行消息推送，即指定client进行发送
    function send($k,$ar,$target='all'){
        $ar['from']=$this->users[$k]['area'];
        $ar['target']=$target;
